package cn.muxiaozi.circle.libgdx.link.stage;
...
import cn.muxiaozi.circle.libgdx.link.DataFactory;
import cn.muxiaozi.circle.libgdx.link.MainGame;
import cn.muxiaozi.circle.libgdx.link.Res;
import cn.muxiaozi.circle.libgdx.link.actor.DiamondActor;
import cn.muxiaozi.circle.libgdx.link.actor.LinkActor;
...
public class GameStage extends BaseStage<MainGame> {
    ...
    public GameStage(final MainGame game, Viewport viewport, int index) {
        ...
        takeTurnSound = game.getAssetManager().get(Res.Audio.TAKETURN, Sound.class);
        ...
        timeActor.setBounds(0, getHeight() - 30, getWidth(), 30);
        ...
        for (int y = 1; y < Res.ROW_NUM - 1; y++) {
            for (int x = 1; x < Res.COL_NUM - 1; x += 2) {
                //第col列
                diamonds[x][y] = new DiamondActor(atlas, (byte) -1, Res.CELL_SIZE - 2);
                diamonds[x][y].setBounds(x * Res.CELL_SIZE + 1, y * Res.CELL_SIZE + 1,
                        Res.CELL_SIZE - 2, Res.CELL_SIZE - 2);
                addActor(diamonds[x][y]);

                //第col+1列
                diamonds[x + 1][y] = new DiamondActor(atlas, (byte) -1, Res.CELL_SIZE - 2);
    ...
    public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        if (isTurnMe) {
            ...
            if (map[touchPoint.x][touchPoint.y] != -1) {
                //正在进行提示
                ...
                if (focusPoint1.equals(-1, -1)) {
                    focusPoint1.set(touchPoint.x, touchPoint.y);
                    diamonds[focusPoint1.x][focusPoint1.y].setFocus(true);
                } else {
                    focusPoint2.set(touchPoint.x, touchPoint.y);
                    if (map[focusPoint1.x][focusPoint1.y] == map[focusPoint2.x][focusPoint2.y] &&
                            canConnected(focusPoint1, focusPoint2, true)) {

                        //给所有玩家发送消除信息
                        getGame().send(DataFactory.packClearDiamonds(new DataFactory.DoubleKillEntity(
                                myIndex, focusPoint1.x, focusPoint1.y, focusPoint2.x, focusPoint2.y
                        )));
     ...
     * @param y2 第二个方块的y坐标
     */
    private void clearDiamonds(int x1, int y1, int x2, int y2) {
        focusPoint1.set(x1, y1);
        focusPoint2.set(x2, y2);
    ...
    public void receive(final byte[] data) {
        switch (data[0]) {
            case DataFactory.TYPE_CLEAR_DIAMONDS:
                final DataFactory.DoubleKillEntity entity = DataFactory.unpackClearDiamonds(data);
                if (entity != null) {
                    Gdx.app.postRunnable(new Runnable() {
                        @Override
                        public void run() {
                            clearDiamonds(entity.x1, entity.y1, entity.x2, entity.y2);
                            playerActor.addGrade(entity.index);
                        }
                    });
                }
                break;
